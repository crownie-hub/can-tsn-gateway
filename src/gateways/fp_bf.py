...
#
# FP-BF enc delay uses fixed-point iteration:
...
def _compute_A_H(tau_ms, hp_periods):
    """
    Higher-priority arrivals in window tau_ms.
...
def _min_tau_for_threshold(threshold, periods, C_src):
    ...
    Uses binary search over arrival times.
    """
    if threshold <= 0:
        return 0.0
...
class FPBatchGateway:
    """
    Fixed-Priority Buffer-Full analytical gateway.

    Enc delay for message m_i via fixed-point iteration:

        delta_enc(m_i) = min{ tau | A(tau) >= (N_H(tau)+1)*n }

    where:
        A(tau)   = total arrivals in window tau
        A_H(tau) = higher-priority arrivals in tau
        N_H(tau) = floor((|M_H| + A_H(tau)) / n)
        |M_H|    = number of HP messages already in buffer

    Fixed-point:
        N_H^(0) = floor(|M_H| / n)
        tau^(k) = min{ tau | A(tau) >= (N_H^(k) + 1)*n }
        N_H^(k+1) = floor((|M_H| + A_H(tau^(k))) / n)
        iterate until convergence.

    Decap delay: delta_dec = (n-1)*C_max (same as FIFO-BF).
    """
...
class FPZeroSlackGateway:
    """
    Fixed-Priority Zero-Slack analytical gateway.

    Enc delay uses the same slack-based bound as FIFO-ZS:
        delta_enc = min(tau_BF, tau_ZS)

    Under ZS the batch fires at L_j = min_i(a_i + S_i)
    before higher-priority displacement can occur, so the
    FP-BF fixed-point iteration is not needed.

    Decap delay: delta_dec = (n-1)*C_max.
    """
